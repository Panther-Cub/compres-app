  ...
  startMonitoring(): void {
    ...
    this.monitoringInterval = setInterval(async () => {
      try {
        ...
        this.emit('thermal-pressure-updated', status.thermalPressure);
        
        const wasThrottling = this.shouldThrottle(status.cpuTemperature, status.cpuUsage);
        if (wasThrottling !== status.isThrottling) {
          this.emit('throttling-changed', wasThrottling);
        }
        
        const action = this.getRecommendedAction(status.cpuTemperature, status.cpuUsage);
        if (action !== 'normal') {
